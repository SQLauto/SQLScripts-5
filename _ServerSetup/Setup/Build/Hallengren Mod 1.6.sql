...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
WARDY IT SOLUTIONS MODIFIED VERSION

Date:		Version:	Editor:			Details
----------------------------------------------------------------------------------------------------------------------------------------------------
??/??/14	v1.?		Aaron Mussig	Rewrite for standardisation of output files, schedules and options. Also several of the smaller weekly tasks were combined into a single job.
07/08/14	v1.?		Hadyn Dawson	Rewrite Output File Cleanup Job. Altered command that cleans up backup files as this was failing.
04/09/14	v1.4		Aaron Mussig	Changed integrity jobs to T-SQL as mini dumps could cause jobs to fail and not report failure. Added manual option to allow user to pick if backup compression should be enabled.
04/09/14	v1.4		Aaron Mussig	Updated actual core hallengren script to latest version (24 Mar 2014) from (22 Sep 2013).
04/09/14	v1.4		Aaron Mussig	Added hash breaks --- WardyIT Solutions --- above and ---------- END ---------- below custom lines of code for ease of upgrade.
05/09/14	v1.4		Aaron Mussig	Added (v1.4) to the end of the job description to track version number.
25/09/14	v1.5		Hadyn Dawson	Commented out "AND @Version < 12" to correct bug where @OutputFileDirectory is not populated when implementing on SQL 2014
25/09/14	v1.5		Hadyn Dawson	Updated version references to v1.5
22/04/15	v1.6		Aaron Mussig	Updated core Hallengren script to latest version (25 Jan 2015). SQL 2014 support and @CleanupMode parameter for cleaning up backups before execution.
...
-- Differential Backups
	Retention: 4 Days
	NOTE: Set the cleanup time for USER FULL backups to 100 hours and adjust the schedule to only execute on Thurs and Sunday
              if performing differential backups.
...
--- WardyIT Solutions ---
DECLARE @ToCompressBackups nvarchar(max)
---------- END ----------

SET @CreateJobs          = 'Y'          -- Specify whether jobs should be created.
SET @BackupDirectory     = N'\\shlfiler01\sqlbackups$' -- Specify the backup root directory.
SET @ToCompressBackups	 = 'Y'			-- Should backup compression be enabled?
...
--- WardyIT Solutions ---
INSERT INTO #Config ([Name], [Value])
VALUES('ToCompressBackups', @ToCompressBackups)
---------- END ----------
...
BEGIN
  ...
  --- WardyIT Solutions ---
  SELECT @ToCompressBackups = Value
  FROM #Config
  WHERE [Name] = 'ToCompressBackups'
  ...
  DECLARE @JobDescription01 nvarchar(max)
  DECLARE @JobDescription02 nvarchar(max)
  DECLARE @JobDescription03 nvarchar(max)
  DECLARE @JobDescription04 nvarchar(max)
  DECLARE @JobDescription05 nvarchar(max)
  DECLARE @JobDescription06 nvarchar(max)
  DECLARE @JobDescription07 nvarchar(max)
  DECLARE @JobDescription08 nvarchar(max)
  DECLARE @JobDescription09 nvarchar(max)
  DECLARE @JobDescription10 nvarchar(max)
  DECLARE @JobDescription11 nvarchar(max)
  DECLARE @JobDescription12 nvarchar(max)
  DECLARE @JobDescription13 nvarchar(max)
  DECLARE @JobDescription14 nvarchar(max)

  DECLARE @OutputFile12 nvarchar(max)

  DECLARE @JobName12 nvarchar(max)
  DECLARE @JobName13 nvarchar(max)
  DECLARE @JobName14 nvarchar(max)

  DECLARE @JobCommand12 nvarchar(max)

  SET @JobDescription01 = 'Performs a full backup of all system databases. WARDY IT SOLUTIONS 2015 (v1.6)'
  SET @JobDescription02 = 'Performs a differential backup of all user databases. WARDY IT SOLUTIONS 2015 (v1.6)'
  SET @JobDescription03 = 'Performs a full backup of all user databases. WARDY IT SOLUTIONS 2015 (v1.6)'
  SET @JobDescription04 = 'Performs a transaction log backup of all user databases. WARDY IT SOLUTIONS 2015 (v1.6)'
  SET @JobDescription05 = 'Performs an integrity check of all system databases. WARDY IT SOLUTIONS 2015 (v1.6)'
  SET @JobDescription06 = 'Performs an integrity check of all user databases. WARDY IT SOLUTIONS 2015 (v1.6)'
  SET @JobDescription07 = 'Performs index maintenance of all user databases, including updating statistics. WARDY IT SOLUTIONS 2015 (v1.6)'
  SET @JobDescription08 = 'Performs a backup history cleanup. WARDY IT SOLUTIONS 2015 (v1.6)'
  SET @JobDescription09 = 'Purges the job history that are outside a specific date range. WARDY IT SOLUTIONS 2015 (v1.6)'
  SET @JobDescription10 = 'Performs a cleanup of all output files. WARDY IT SOLUTIONS 2015 (v1.6)'
  SET @JobDescription11 = 'Removes logs from the default master.dbo.CommandLog table that are outside a specific date range. WARDY IT SOLUTIONS 2015 (v1.6)'
  SET @JobDescription12 = 'Archives the current SQL Server log file and creates a new log file to store events. WARDY IT SOLUTIONS 2015 (v1.6)'
  SET @JobDescription13 = 'Performs an integrity check of all user and system databases. WARDY IT SOLUTIONS 2015 (v1.6)'
  SET @JobDescription14 = 'Performs weekly maintenance tasks for Command Log Cleanup, Cycle Error Log, Delete Backup History, Output File Cleanup and Purging the job history. WARDY IT SOLUTIONS 2015 (v1.6)'


  SET @JobName01 = 'ADMIN-Database Backup SYSTEM FULL'
  SET @JobCommand01 = 'sqlcmd -E -S ' + @TokenServer + ' -d ' + @DatabaseName + ' -Q "EXECUTE [dbo].[DatabaseBackup] @Databases = ''SYSTEM_DATABASES'', @Directory = ' + ISNULL('N''' + REPLACE(@BackupDirectory,'''','''''') + '''','NULL') + ', @BackupType = ''FULL'', @Verify = ''Y'', @CleanupTime = 102, @Compress = ''' + @ToCompressBackups + ''', @ChangeBackupType = ''Y'', @CheckSum = ''Y''' + CASE WHEN @LogToTable = 'Y' THEN ', @LogToTable = ''Y''' ELSE '' END + '" -b'
  SET @OutputFile01 = @OutputFileDirectory + '\' + 'ADMIN-Database Backup SYSTEM FULL_' + @TokenDate + '_' + @TokenTime + '.txt'
  ...
  SET @JobName02 = 'ADMIN-Database Backup USER DIFF'
  SET @JobCommand02 = 'sqlcmd -E -S ' + @TokenServer + ' -d ' + @DatabaseName + ' -Q "EXECUTE [dbo].[DatabaseBackup] @Databases = ''USER_DATABASES'', @Directory = ' + ISNULL('N''' + REPLACE(@BackupDirectory,'''','''''') + '''','NULL') + ', @BackupType = ''DIFF'', @Verify = ''Y'', @CleanupTime = 100, @Compress = ''' + @ToCompressBackups + ''', @ChangeBackupType = ''Y'', @CheckSum = ''Y''' + CASE WHEN @LogToTable = 'Y' THEN ', @LogToTable = ''Y''' ELSE '' END + '" -b'
  SET @OutputFile02 = @OutputFileDirectory + '\' + 'ADMIN-Database Backup USER DIFF_' + @TokenDate + '_' + @TokenTime + '.txt'
  ...
  SET @JobName03 = 'ADMIN-Database Backup USER FULL'
  SET @JobCommand03 = 'sqlcmd -E -S ' + @TokenServer + ' -d ' + @DatabaseName + ' -Q "EXECUTE [dbo].[DatabaseBackup] @Databases = ''USER_DATABASES'', @Directory = ' + ISNULL('N''' + REPLACE(@BackupDirectory,'''','''''') + '''','NULL') + ', @BackupType = ''FULL'', @Verify = ''Y'', @CleanupTime = 54, @Compress = ''' + @ToCompressBackups + ''', @ChangeBackupType = ''Y'', @CheckSum = ''Y''' + CASE WHEN @LogToTable = 'Y' THEN ', @LogToTable = ''Y''' ELSE '' END + '" -b'
  SET @OutputFile03 = @OutputFileDirectory + '\' + 'ADMIN-Database Backup USER FULL_' + @TokenDate + '_' + @TokenTime + '.txt'
  ...
  SET @JobName04 = 'ADMIN-TLog Backup'
  SET @JobCommand04 = 'sqlcmd -E -S ' + @TokenServer + ' -d ' + @DatabaseName + ' -Q "EXECUTE [dbo].[DatabaseBackup] @Databases = ''USER_DATABASES'', @Directory = ' + ISNULL('N''' + REPLACE(@BackupDirectory,'''','''''') + '''','NULL') + ', @BackupType = ''LOG'', @Verify = ''Y'', @CleanupTime = 72, @Compress = ''' + @ToCompressBackups + ''', @ChangeBackupType = ''Y'', @CheckSum = ''Y''' + CASE WHEN @LogToTable = 'Y' THEN ', @LogToTable = ''Y''' ELSE '' END + '" -b'
  SET @OutputFile04 = @OutputFileDirectory + '\' + 'ADMIN-Database Backup USER LOG_' + @TokenDate + '_' + @TokenTime + '.txt'
  ...
  SET @JobName05 = 'ADMIN-Integrity Check SYSTEM'
  SET @JobCommand05 = 'EXECUTE [dbo].[DatabaseIntegrityCheck] @Databases = ''SYSTEM_DATABASES''' + CASE WHEN @LogToTable = 'Y' THEN ', @LogToTable = ''Y''' ELSE '' END + ''
  SET @OutputFile05 = @OutputFileDirectory + '\' + 'ADMIN-Integrity Check SYSTEM_' + @TokenDate + '_' + @TokenTime + '.txt'
  ...
  SET @JobName06 = 'ADMIN-Integrity Check USER'
  SET @JobCommand06 = 'EXECUTE [dbo].[DatabaseIntegrityCheck] @Databases = ''USER_DATABASES''' + CASE WHEN @LogToTable = 'Y' THEN ', @LogToTable = ''Y''' ELSE '' END + ''
  SET @OutputFile06 = @OutputFileDirectory + '\' + 'ADMIN-Integrity Check USER_' + @TokenDate + '_' + @TokenTime + '.txt'
  ...
  SET @JobName07 = 'ADMIN-Index Maintenance'
  SET @JobCommand07 = 'sqlcmd -E -S ' + @TokenServer + ' -d ' + @DatabaseName + ' -Q "EXECUTE [dbo].[IndexOptimize] @Databases = ''USER_DATABASES'', @FragmentationLow = NULL, @FragmentationMedium = ''INDEX_REORGANIZE,INDEX_REBUILD_ONLINE,INDEX_REBUILD_OFFLINE'', @FragmentationHigh = ''INDEX_REBUILD_ONLINE,INDEX_REBUILD_OFFLINE'', @FragmentationLevel1 = 15, @FragmentationLevel2 = 30, @UpdateStatistics = ''ALL''' + CASE WHEN @LogToTable = 'Y' THEN ', @LogToTable = ''Y''' ELSE '' END + '" -b'
  SET @OutputFile07 = @OutputFileDirectory + '\' + 'ADMIN-Index Maintenance USER_' + @TokenDate + '_' + @TokenTime + '.txt'
  ...
  SET @JobName08 = 'ADMIN-Delete Backup History'
  ...
  SET @OutputFile08 = @OutputFileDirectory + '\' + 'ADMIN-Delete Backup History_' + @TokenDate + '_' + @TokenTime + '.txt'
  ...
  SET @JobName09 = 'ADMIN-Purge Job History'
  ...
  SET @OutputFile09 = @OutputFileDirectory + '\' + 'ADMIN-Purge Job History_' + @TokenDate + '_' + @TokenTime + '.txt'
  ...
  SET @JobName10 = 'ADMIN-Output File Cleanup'
  SET @JobCommand10 = 'cmd /q /c "For /F "tokens=1 delims=" %v In (''ForFiles /P "' + @OutputFileDirectory + '" /m "ADMIN-*.txt" /d -30 2^>^&1'') do if EXIST "' + @OutputFileDirectory + '"\%v echo del "' + @OutputFileDirectory + '"\%v& del "' + @OutputFileDirectory + '"\%v"
cmd /q /c "For /F "tokens=1 delims=" %v In (''ForFiles /P "' + @OutputFileDirectory + '" /m "ADMIN-Database Backup USER LOG*.txt" /d -7 2^>^&1'') do if EXIST "' + @OutputFileDirectory + '"\%v echo del "' + @OutputFileDirectory + '"\%v& del "' + @OutputFileDirectory + '"\%v"'
  SET @OutputFile10 = @OutputFileDirectory + '\' + 'ADMIN-Output File Cleanup_' + @TokenDate + '_' + @TokenTime + '.txt'
  ...
  SET @JobName11 = 'ADMIN-Command Log Cleanup'
  ...
  SET @OutputFile11 = @OutputFileDirectory + '\' + 'ADMIN-Command Log Cleanup_' + @TokenDate + '_' + @TokenTime + '.txt'
  ...
  SET @JobName12 = 'ADMIN-Cycle ErrorLog'
  SET @JobCommand12 = 'sqlcmd -E -S ' + @TokenServer + ' -d ' + 'master' + ' -Q "EXEC sp_cycle_errorlog" -b' 
  SET @OutputFile12 = @OutputFileDirectory + '\' + 'ADMIN-Cycle ErrorLog_' + @TokenDate + '_' + @TokenTime + '.txt'
  ...
  SET @JobName13 = 'ADMIN-Integrity Checks'
  SET @JobName14 = 'ADMIN-Weekly Maintenance'

  IF NOT EXISTS (SELECT * FROM msdb.dbo.sysjobs WHERE [name] = @JobName01)
  BEGIN
    EXECUTE msdb.dbo.sp_add_job @job_name = @JobName01, @description = @JobDescription01, @category_name = @JobCategory, @owner_login_name = @JobOwner, @enabled = 1
    ...
    EXECUTE msdb.dbo.sp_add_schedule @schedule_name = @JobName01, @freq_type = 4, @freq_interval = 1, @active_start_time = 223000
	EXECUTE msdb.dbo.sp_attach_schedule @job_name = @JobName01, @schedule_name = @JobName01
	EXECUTE msdb.dbo.sp_add_jobserver @job_name = @JobName01
  END

  IF NOT EXISTS (SELECT * FROM msdb.dbo.sysjobs WHERE [name] = @JobName02)
  BEGIN
    EXECUTE msdb.dbo.sp_add_job @job_name = @JobName02, @description = @JobDescription02, @category_name = @JobCategory, @owner_login_name = @JobOwner, @enabled = 0
    ...
    EXECUTE msdb.dbo.sp_add_schedule @schedule_name = @JobName02, @freq_type = 8, @freq_interval = 110, @freq_recurrence_factor = 1, @active_start_time = 230000
	EXECUTE msdb.dbo.sp_attach_schedule @job_name = @JobName02, @schedule_name = @JobName02
	EXECUTE msdb.dbo.sp_add_jobserver @job_name = @JobName02
  END

  IF NOT EXISTS (SELECT * FROM msdb.dbo.sysjobs WHERE [name] = @JobName03)
  BEGIN
    EXECUTE msdb.dbo.sp_add_job @job_name = @JobName03, @description = @JobDescription03, @category_name = @JobCategory, @owner_login_name = @JobOwner, @enabled = 1
    ...
    EXECUTE msdb.dbo.sp_add_schedule @schedule_name = @JobName03, @freq_type = 4, @freq_interval = 1, @active_start_time = 230000
	EXECUTE msdb.dbo.sp_attach_schedule @job_name = @JobName03, @schedule_name = @JobName03
	EXECUTE msdb.dbo.sp_add_jobserver @job_name = @JobName03
  END

  IF NOT EXISTS (SELECT * FROM msdb.dbo.sysjobs WHERE [name] = @JobName04)
  BEGIN
    EXECUTE msdb.dbo.sp_add_job @job_name = @JobName04, @description = @JobDescription04, @category_name = @JobCategory, @owner_login_name = @JobOwner, @enabled = 1
    EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName04, @step_name = @JobName04, @subsystem = 'CMDEXEC', @command = @JobCommand04, @output_file_name = @OutputFile04, @retry_attempts = 2, @retry_interval = 1
    EXECUTE msdb.dbo.sp_add_schedule @schedule_name = @JobName04, @freq_type = 4, @freq_interval = 1, @freq_subday_type =0x4, @freq_subday_interval = 15, @active_start_time = 000000, @active_end_time = 235959
	EXECUTE msdb.dbo.sp_attach_schedule @job_name = @JobName04, @schedule_name = @JobName04   
	EXECUTE msdb.dbo.sp_add_jobserver @job_name = @JobName04
  END

   -- IF NOT EXISTS (SELECT * FROM msdb.dbo.sysjobs WHERE [name] = @JobName05)
 -- BEGIN
 --   EXECUTE msdb.dbo.sp_add_job @job_name = @JobName05, @description = @JobDescription05, @category_name = @JobCategory, @owner_login_name = @JobOwner, @enabled = 0
 ...
 --   EXECUTE msdb.dbo.sp_add_schedule @schedule_name = @JobName05, @freq_type = 8, @freq_interval = 1, @freq_recurrence_factor = 1, @active_start_time = 195000
	--EXECUTE msdb.dbo.sp_attach_schedule @job_name = @JobName05, @schedule_name = @JobName05
	--EXECUTE msdb.dbo.sp_add_jobserver @job_name = @JobName05
 -- END

 -- IF NOT EXISTS (SELECT * FROM msdb.dbo.sysjobs WHERE [name] = @JobName06)
 -- BEGIN
 --   EXECUTE msdb.dbo.sp_add_job @job_name = @JobName06, @description = @JobDescription06, @category_name = @JobCategory, @owner_login_name = @JobOwner, @enabled = 0
 ...
 --   EXECUTE msdb.dbo.sp_add_schedule @schedule_name = @JobName06, @freq_type = 8, @freq_interval = 1, @freq_recurrence_factor = 1, @active_start_time = 200000
	--EXECUTE msdb.dbo.sp_attach_schedule @job_name = @JobName06, @schedule_name = @JobName06
	--EXECUTE msdb.dbo.sp_add_jobserver @job_name = @JobName06
 -- END

  IF NOT EXISTS (SELECT * FROM msdb.dbo.sysjobs WHERE [name] = @JobName07)
  BEGIN
    EXECUTE msdb.dbo.sp_add_job @job_name = @JobName07, @description = @JobDescription07, @category_name = @JobCategory, @owner_login_name = @JobOwner, @enabled = 1
    ...
    EXECUTE msdb.dbo.sp_add_schedule @schedule_name = @JobName07, @freq_type = 8, @freq_interval = 64, @freq_recurrence_factor = 1, @active_start_time = 200000
	EXECUTE msdb.dbo.sp_attach_schedule @job_name = @JobName07, @schedule_name = @JobName07
    EXECUTE msdb.dbo.sp_add_jobserver @job_name = @JobName07
  END

 -- IF NOT EXISTS (SELECT * FROM msdb.dbo.sysjobs WHERE [name] = @JobName08)
 -- BEGIN
 --   EXECUTE msdb.dbo.sp_add_job @job_name = @JobName08, @description = @JobDescription08, @category_name = @JobCategory, @owner_login_name = @JobOwner, @enabled = 0
 --   EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName08, @step_name = @JobName08, @subsystem = 'CMDEXEC', @command = @JobCommand08, @output_file_name = @OutputFile08, @retry_attempts = 3, @retry_interval = 5
 --   EXECUTE msdb.dbo.sp_add_schedule @schedule_name = @JobName08, @freq_type = 8, @freq_interval = 64, @freq_recurrence_factor = 1, @active_start_time = 021000
	--EXECUTE msdb.dbo.sp_attach_schedule @job_name = @JobName08, @schedule_name = @JobName08
	--EXECUTE msdb.dbo.sp_add_jobserver @job_name = @JobName08
 -- END

 -- IF NOT EXISTS (SELECT * FROM msdb.dbo.sysjobs WHERE [name] = @JobName09)
 -- BEGIN
 --   EXECUTE msdb.dbo.sp_add_job @job_name = @JobName09, @description = @JobDescription09, @category_name = @JobCategory, @owner_login_name = @JobOwner, @enabled = 0
 --   EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName09, @step_name = @JobName09, @subsystem = 'CMDEXEC', @command = @JobCommand09, @output_file_name = @OutputFile09, @retry_attempts = 3, @retry_interval = 5
 --   EXECUTE msdb.dbo.sp_add_schedule @schedule_name = @JobName09, @freq_type = 8, @freq_interval = 64, @freq_recurrence_factor = 1, @active_start_time = 022000
	--EXECUTE msdb.dbo.sp_attach_schedule @job_name = @JobName09, @schedule_name = @JobName09
	--EXECUTE msdb.dbo.sp_add_jobserver @job_name = @JobName09
 -- END

 -- IF NOT EXISTS (SELECT * FROM msdb.dbo.sysjobs WHERE [name] = @JobName10)
 -- BEGIN
 --   EXECUTE msdb.dbo.sp_add_job @job_name = @JobName10, @description = @JobDescription10, @category_name = @JobCategory, @owner_login_name = @JobOwner, @enabled = 0
 --   EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName10, @step_name = @JobName10, @subsystem = 'CMDEXEC', @command = @JobCommand10, @output_file_name = @OutputFile10, @retry_attempts = 3, @retry_interval = 5
 --   EXECUTE msdb.dbo.sp_add_schedule @schedule_name = @JobName10, @freq_type = 8, @freq_interval = 64, @freq_recurrence_factor = 1, @active_start_time = 021500
	--EXECUTE msdb.dbo.sp_attach_schedule @job_name = @JobName10, @schedule_name = @JobName10
	--EXECUTE msdb.dbo.sp_add_jobserver @job_name = @JobName10
 -- END

 -- IF NOT EXISTS (SELECT * FROM msdb.dbo.sysjobs WHERE [name] = @JobName11)
 -- BEGIN
 --   EXECUTE msdb.dbo.sp_add_job @job_name = @JobName11, @description = @JobDescription11, @category_name = @JobCategory, @owner_login_name = @JobOwner, @enabled = 0
 --   EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName11, @step_name = @JobName11, @subsystem = 'CMDEXEC', @command = @JobCommand11, @output_file_name = @OutputFile11, @retry_attempts = 3, @retry_interval = 5
 --   EXECUTE msdb.dbo.sp_add_schedule @schedule_name = @JobName11, @freq_type = 8, @freq_interval = 64, @freq_recurrence_factor = 1, @active_start_time = 020000
	--EXECUTE msdb.dbo.sp_attach_schedule @job_name = @JobName11, @schedule_name = @JobName11
	--EXECUTE msdb.dbo.sp_add_jobserver @job_name = @JobName11
 -- END

 --   IF NOT EXISTS (SELECT * FROM msdb.dbo.sysjobs WHERE [name] = @JobName12)
 -- BEGIN
 --   EXECUTE msdb.dbo.sp_add_job @job_name = @JobName12, @description = @JobDescription12, @category_name = @JobCategory, @owner_login_name = @JobOwner, @enabled = 0
 --   EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName12, @step_name = @JobName12, @subsystem = 'CMDEXEC', @command = @JobCommand12, @output_file_name = @OutputFile12, @retry_attempts = 3, @retry_interval = 5
	--EXECUTE msdb.dbo.sp_add_schedule @schedule_name = @JobName12, @freq_type = 8, @freq_interval = 64, @freq_recurrence_factor = 1, @active_start_time = 020500
	--EXECUTE msdb.dbo.sp_attach_schedule @job_name = @JobName12, @schedule_name = @JobName12
	--EXECUTE msdb.dbo.sp_add_jobserver @job_name = @JobName12
 -- END

      IF NOT EXISTS (SELECT * FROM msdb.dbo.sysjobs WHERE [name] = @JobName13)
  BEGIN
    EXECUTE msdb.dbo.sp_add_job @job_name = @JobName13, @description = @JobDescription13, @category_name = @JobCategory, @owner_login_name = @JobOwner, @enabled = 1
    EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName13, @step_name = @JobName05, @subsystem = 'TSQL', @command = @JobCommand05, @output_file_name = @OutputFile05, @on_success_action=3
    EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName13, @step_name = @JobName06, @subsystem = 'TSQL', @command = @JobCommand06, @output_file_name = @OutputFile06
	EXECUTE msdb.dbo.sp_add_schedule @schedule_name = @JobName13, @freq_type = 8, @freq_interval = 17, @freq_recurrence_factor = 1, @active_start_time = 200000
	EXECUTE msdb.dbo.sp_attach_schedule @job_name = @JobName13, @schedule_name = @JobName13
	EXECUTE msdb.dbo.sp_add_jobserver @job_name = @JobName13
  END

	IF NOT EXISTS (SELECT * FROM msdb.dbo.sysjobs WHERE [name] = @JobName14)
  BEGIN
    EXECUTE msdb.dbo.sp_add_job @job_name = @JobName14, @description = @JobDescription14, @category_name = @JobCategory, @owner_login_name = @JobOwner, @enabled = 1
    EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName14, @step_name = @JobName09, @subsystem = 'CMDEXEC', @command = @JobCommand09, @output_file_name = @OutputFile09, @retry_attempts = 3, @retry_interval = 5, @on_success_action=3
	EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName14, @step_name = @JobName10, @subsystem = 'CMDEXEC', @command = @JobCommand10, @output_file_name = @OutputFile10, @retry_attempts = 3, @retry_interval = 5, @on_success_action=3
	EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName14, @step_name = @JobName11, @subsystem = 'CMDEXEC', @command = @JobCommand11, @output_file_name = @OutputFile11, @retry_attempts = 3, @retry_interval = 5, @on_success_action=3
	EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName14, @step_name = @JobName12, @subsystem = 'CMDEXEC', @command = @JobCommand12, @output_file_name = @OutputFile12, @retry_attempts = 3, @retry_interval = 5, @on_success_action=3
	EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName14, @step_name = @JobName08, @subsystem = 'CMDEXEC', @command = @JobCommand08, @output_file_name = @OutputFile08, @retry_attempts = 3, @retry_interval = 5
    EXECUTE msdb.dbo.sp_add_schedule @schedule_name = @JobName14, @freq_type = 8, @freq_interval = 64, @freq_recurrence_factor = 1, @active_start_time = 020000
	EXECUTE msdb.dbo.sp_attach_schedule @job_name = @JobName14, @schedule_name = @JobName14
	EXECUTE msdb.dbo.sp_add_jobserver @job_name = @JobName14
  END

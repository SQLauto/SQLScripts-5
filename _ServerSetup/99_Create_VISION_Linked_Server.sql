...
SET @RealName = ISNULL((SELECT REPLACE(CAST(ServerProperty('ServerName') as nvarchar(128)), 'MRI', 'VS')+'\SQL2K8R2'), N'DEVVSSQL01')
SET @SrvrName = N'VISION'
...
EXEC master.dbo.sp_addlinkedserver @server = @SrvrName
  ...
  , @datasrc = @RealName, @catalog = N'KnightVision'

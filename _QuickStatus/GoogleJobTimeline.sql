...
declare @EmailRecipients varchar(250)
...
set @SendMail = 1
set @Emailprofilename = 'default'
set @EmailRecipients = 'Phil.Carter@au.knightfrank.com;gwood@au.knightfrank.com'

--***************************************************************************************
-- Pre-run cleanup (just in case)
--***************************************************************************************
IF OBJECT_ID('tempdb..#JobRuntime') IS NOT NULL DROP TABLE #JobRuntime;
IF OBJECT_ID('tempdb..GoogleGraph') IS NOT NULL DROP TABLE TempDB..GoogleGraph;

--***************************************************************************************
-- Create a table for HTML assembly
--***************************************************************************************
create table TempDB..GoogleGraph ([ID] [int] IDENTITY(1,1) NOT NULL,
...
--***************************************************************************************
insert into TempDB..GoogleGraph (HTML) 
select '<html>
	<head>
...
if @ReturnRecocordset = 1 
	select html from TempDB..GoogleGraph order by ID
...
if @SendMail = 1 
	execute msdb.dbo.sp_send_dbmail	
		...
		,@query = 'set nocount on; SELECT HTML FROM TempDB..GoogleGraph'
		...
		,@query_attachment_filename= 'JobTimeline.htm'

...
/*********************************************************************************************
Who Is Active? v9.57 (2009-11-24)
...
BEGIN
	...
	BEGIN;
									...
									WHEN 'session' THEN
										CASE
											WHEN
												...
												OR sp.spid = CONVERT(TINYINT, @filter)
													THEN 1
											ELSE 0
										END
									WHEN 'program' THEN
						...
						CASE
							WHEN @not_filter <> '' THEN
								CASE @not_filter_type
									WHEN 'session' THEN
										CASE
											WHEN sp.spid = CONVERT(TINYINT, @not_filter) THEN 1
											ELSE 0
										END
									WHEN 'program' THEN
										CASE
		...
		SET @sql = 
			...
			(
				...
				'( ' +
					...
					'blocked SMALLINT, ' +
					'UNIQUE CLUSTERED (session_id, kpid) ' +
				'); ' +
				'' +
				...
				'( ' +
					'SELECT ' +
						...
						'sp1.stmt_end, ' +
						'sp1.sql_handle, ' +
						'CONVERT(NVARCHAR(128), RTRIM(sp1.hostname)) COLLATE SQL_Latin1_General_CP1_CI_AS AS host_name, ' +
						'MAX(CONVERT(NVARCHAR(128), RTRIM(sp1.loginame)) COLLATE SQL_Latin1_General_CP1_CI_AS) OVER (PARTITION BY sp1.spid, sp1.request_id) AS login_name, ' +
						'CONVERT(NVARCHAR(128), RTRIM(sp1.program_name)) COLLATE SQL_Latin1_General_CP1_CI_AS AS program_name, ' +
									...
									WHEN 'session' THEN
										CASE
											WHEN CONVERT(TINYINT, @filter) <> 0 THEN
												'AND sp0.session_id = CONVERT(TINYINT, @filter) '
											ELSE ''
										END
									WHEN 'program' THEN
						...
						CASE
							WHEN @not_filter <> '' THEN
								CASE @not_filter_type
									WHEN 'session' THEN
										CASE
											WHEN CONVERT(TINYINT, @not_filter) <> 0 THEN
												'AND sp0.session_id <> CONVERT(TINYINT, @not_filter) '
											ELSE ''
										END
									WHEN 'program' THEN
			...
			(
				...
				'( ' +
					'SELECT ' +
						...
						'COALESCE(CONVERT(BIGINT, mg.used_memory_kb / 8), 0) + y.memory_usage AS used_memory, ' +
						CASE 
					...
					'( ' +
						'SELECT ' +
							...
							'( ' +
								CASE 
									WHEN NOT (@get_transaction_info = 1 AND @recursion = 1) THEN 'SELECT CONVERT(VARCHAR(4000), NULL) '
									ELSE
										'SELECT ' +
											'REPLACE ' +
											'( ' +
				...
				(
					...
					CASE 
						WHEN 
							...
							'( ' +
								'SELECT ' +
									...
									'CONVERT(VARCHAR(8000), NULL) AS wait_info ' +
							') AS tasks '
						WHEN @get_task_info = 2 THEN
							...
							'( ' +
								'SELECT ' +
									...
									'task_nodes.task_node.value(''(waits/text())[1]'', ''VARCHAR(8000)'') AS wait_info ' +
								'FROM ' +
								'( ' +
									'SELECT ' +
										'CONVERT ' +
										...
										') AS task_xml, tasks_raw.task_xml_raw ' +
									'FROM ' +
									'( ' +
										'SELECT ' +
											'CASE waits.r ' +
										...
										'( ' +
											...
											'( ' +
												...
												'( ' +
													...
													'INNER HASH JOIN sys.dm_os_threads AS th ON ' +
														'th.os_thread_id = sp2.kpid ' +
													'INNER HASH JOIN sys.dm_os_workers AS w ON ' +
															...
															'wt.wait_type + ' +
																...
																'CASE ' +
																	...
																	'WHEN wt.wait_type = ''CXPACKET'' THEN ' +
																		''':'' + SUBSTRING(wt.resource_description, CHARINDEX(''nodeId'', wt.resource_description) + 7, 4)' +
																	'ELSE '''' ' +
												...
												') AS wt2 ON ' +
													...
													'( ' +
														'( ' +
															'task_info.task_state = ''SUSPENDED'' ' +
						...
						ELSE --@get_task_info = 1 OR @get_blockers = 1
							'LEFT OUTER HASH JOIN ' +
							'( ' +
								'SELECT ' +
									'w1.session_id, ' +
									...
									'''('' + CONVERT(VARCHAR, w1.wait_duration_ms) + ''ms)'' + ' +
										'w1.wait_type + ' +
											--TODO: What else can be pulled from the resource_description?
											'CASE ' +
												'WHEN w1.wait_type LIKE ''PAGE%LATCH_%'' THEN ' +
													''':'' + ' +
													--database name
													'COALESCE(DB_NAME(CONVERT(INT, LEFT(w1.resource_description, CHARINDEX('':'', w1.resource_description) - 1))), ''(null)'') + ' +
													''':'' + ' +
													--file id
													'SUBSTRING(w1.resource_description, CHARINDEX('':'', w1.resource_description) + 1, LEN(w1.resource_description) - CHARINDEX('':'', REVERSE(w1.resource_description)) - CHARINDEX('':'', w1.resource_description)) + ' +
													--page # for special pages
													'''('' + ' +
														'CASE ' +
															'WHEN ' +
																'CONVERT(INT, RIGHT(w1.resource_description, CHARINDEX('':'', REVERSE(w1.resource_description)) - 1)) = 1 OR ' +
																'CONVERT(INT, RIGHT(w1.resource_description, CHARINDEX('':'', REVERSE(w1.resource_description)) - 1)) % 8088 = 0 THEN ''PFS'' ' +
															'WHEN ' +
																'CONVERT(INT, RIGHT(w1.resource_description, CHARINDEX('':'', REVERSE(w1.resource_description)) - 1)) = 2 OR ' +
																'CONVERT(INT, RIGHT(w1.resource_description, CHARINDEX('':'', REVERSE(w1.resource_description)) - 1)) % 511232 = 0 THEN ''GAM'' ' +
															'WHEN ' +
																'CONVERT(INT, RIGHT(w1.resource_description, CHARINDEX('':'', REVERSE(w1.resource_description)) - 1)) = 3 OR ' +
																'CONVERT(INT, RIGHT(w1.resource_description, CHARINDEX('':'', REVERSE(w1.resource_description)) - 1)) % 511233 = 0 THEN ''SGAM'' ' +
															'WHEN ' +
																'CONVERT(INT, RIGHT(w1.resource_description, CHARINDEX('':'', REVERSE(w1.resource_description)) - 1)) = 6 OR ' +
																'CONVERT(INT, RIGHT(w1.resource_description, CHARINDEX('':'', REVERSE(w1.resource_description)) - 1)) % 511238 = 0 THEN ''DCM'' ' +
															'WHEN ' +
																'CONVERT(INT, RIGHT(w1.resource_description, CHARINDEX('':'', REVERSE(w1.resource_description)) - 1)) = 7 OR ' +
																'CONVERT(INT, RIGHT(w1.resource_description, CHARINDEX('':'', REVERSE(w1.resource_description)) - 1)) % 511239 = 0 THEN ''BCM'' ' +
															'ELSE ''*'' ' +
														'END + ' +
													''')'' ' +
												'WHEN w1.wait_type = ''CXPACKET'' THEN ' +
													''':'' + SUBSTRING(w1.resource_description, CHARINDEX(''nodeId'', w1.resource_description) + 7, 4)' +
												'ELSE '''' ' +
								...
								'( ' +
									'SELECT ' +
										...
										'CASE ' +
											...
											'ELSE NULL ' +
										'END AS blocking_session_id, ' +
										'ROW_NUMBER() OVER (PARTITION BY sp2.session_id, sp2.request_id ORDER BY NULLIF(ABS(wt0.session_id - wt0.blocking_session_id), 0) DESC, wt0.wait_duration_ms DESC) AS r ' +
									'FROM sys.dm_os_tasks AS t ' +
									'INNER LOOP JOIN @sessions AS sp2 ON ' +
										'sp2.session_id = t.session_id ' +
										'AND t.task_state = ''SUSPENDED'' ' +
										'AND sp2.status = ''suspended'' ' +
									'INNER HASH JOIN sys.dm_os_waiting_tasks AS wt0 ON ' +
										'wt0.waiting_task_address = t.task_address ' +
										'AND wt0.wait_type <> ''CXPACKET'' ' +
									'INNER HASH JOIN sys.dm_os_threads AS th ON ' +
										'th.os_thread_id = sp2.kpid ' +
									'INNER HASH JOIN sys.dm_os_workers AS w ON ' +
										'w.thread_address = th.thread_address ' +
					...
					(
						...
						'( ' +
							'SELECT ' +
								't_info.session_id, ' +
								't_info.request_id, ' +
								'SUM(t_info.tempdb_writes) AS tempdb_writes, ' +
						...
						'( ' +
							...
							'( ' +
								...
								'FROM @sessions AS s0 ' +
								'WHERE ' +
									's0.session_id = qmg.session_id ' +
							') AS p ' +
							'GROUP BY ' +
								'qmg.session_id, ' +
								'qmg.request_id ' +
						') AS mg ON ' +
							'y.session_id = mg.session_id ' +
							'AND y.request_id = mg.request_id '
					) +
					CASE 
						WHEN NOT (@get_avg_time = 1 AND @recursion = 1) THEN ''
						ELSE
							'LEFT OUTER HASH JOIN sys.dm_exec_query_stats AS qs ON ' +
		...
		EXEC sp_executesql 
			@sql_n,
			N'@recursion SMALLINT, @filter VARCHAR(128), @not_filter VARCHAR(128)',
									...
									REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(
										...
										(
											...
											CASE 
												...
												WHEN SUBSTRING(CONVERT(VARCHAR(MAX), est.text), (@statement_start_offset/2), 2) LIKE '[a-zA-Z0-9][a-zA-Z0-9]' THEN est.text
												ELSE
													CASE
